function []= ParamsMastCam2(A_ML,H_ML,V_ML,A_MR,H_MR,V_MR,C_ML,C_MR,R_ML,R_MR)
...
% All intrinsic and Extrinsic Computations and equations obtained from
% following paper : https://agupubs.onlinelibrary.wiley.com/doi/10.1029/2003JE002199
%% Input Parameters MASTCAM
% pixel size in m
Pixel_size = 0.0000074; 
% Image size
img_size = [1200,1648];
image_height =1200;
image_width = 1648;
...
%% Extrinsics
rotation = Rot_Left*inv(Rot_Right);
tRight =C_MR ;
tLeft = C_ML;
...
Left_int = cameraIntrinsics([f_L/Pixel_size,f_L/Pixel_size],[x0l,y0l],[1200 1648]);
Right_int = cameraIntrinsics([f_R/Pixel_size,f_R/Pixel_size],[x0r,y0r],[1200 1648]);

%radialDistortionL = [k0L,k1L,k2L];
radialDistortionL = [0,0,0];

CameraParamsLM = cameraParameters('IntrinsicMatrix',Left_int.IntrinsicMatrix,...
...
radialDistortionR =[0,0,0]; 
CameraParamsRM= cameraParameters('IntrinsicMatrix',Right_int.IntrinsicMatrix,'ImageSize',img_size,'RadialDistortion',radialDistortionR,'RotationVectors',Rot_Right,'TranslationVectors',diag(tRight)'); 
