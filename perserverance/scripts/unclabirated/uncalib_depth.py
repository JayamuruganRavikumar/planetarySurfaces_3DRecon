...
imgL = cv2.imread("lefrncam_57.png", cv2.IMREAD_GRAYSCALE)  # left image
imgR = cv2.imread("rightrncam_57.png", cv2.IMREAD_GRAYSCALE)  # right image
...
h2, w2 = imgR.shape
thresh = 0
points1a = np.float32(points1)
points2a = np.float32(points2)
_, H1, H2 = cv2.stereoRectifyUncalibrated(
    points1a.transpose().reshape((-1)), points2a.transpose().reshape((-1)), F, imgSize=(h1, w1))

############## Undistort (Rectify) ##############
imgL_undistorted = cv2.warpPerspective(imgL, H1, (w1, 1))
